// First it does a "chromatic aberration" by splitting the rgb signals by a
// product of sin functions over time, then it does a glow effect in a
// perceptual color space Based on kalgynirae's Ghostty passable glow shader and
// NickWest's Chromatic Aberration shader demo Passable glow:
// https://github.com/kalgynirae/dotfiles/blob/main/ghostty/glow.glsl "Chromatic
// Aberration": https://www.shadertoy.com/view/Mds3zn

// sRGB linear -> nonlinear transform from
...
// Oklab <-> linear sRGB conversions from
...
vec4 toOklab(vec4 rgb) {
  ...
  return vec4(0.2104542553 * l_ + 0.7936177850 * m_ - 0.0040720468 * s_,
              ...
              0.0259040371 * l_ + 0.7827717662 * m_ - 0.8086757660 * s_, rgb.a);
}
...
vec4 toRgb(vec4 oklab) {
  ...
  float s = s_ * s_ * s_;
  vec3 linear_srgb =
      vec3(4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s,
           -1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s,
           -0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s);
  return vec4(clamp(f(linear_srgb.r), 0.0, 1.0),
              clamp(f(linear_srgb.g), 0.0, 1.0),
              clamp(f(linear_srgb.b), 0.0, 1.0), oklab.a);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Sample with chromatic aberration and preserve alpha
  vec4 rSample = texture(
      iChannel0, vec2(uv.x - ABBERATION_FACTOR * amount / iResolution.x, uv.y));
  vec4 gSample = texture(iChannel0, uv);
  vec4 bSample = texture(
      iChannel0, vec2(uv.x + ABBERATION_FACTOR * amount / iResolution.x, uv.y));

  vec3 col = vec3(rSample.r, gSample.g, bSample.b);
  float alpha = gSample.a; // Use the center sample's alpha

  vec4 splittedColor = vec4(col, alpha);
  vec4 source = toOklab(splittedColor);
  vec4 dest = source;

  // Only apply glow effects to visible (non-transparent) pixels
  if (alpha > 0.01) {
    if (source.x > DIM_CUTOFF) {
      ...
      for (int i = 0; i < 24; i++) {
        ...
        if (c.x > DIM_CUTOFF && c.a > 0.01) {
          glow.yz += c.yz * weight * 0.15;
